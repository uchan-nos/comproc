...
test_value 05 'int main() {int *p = 30; while(*p == -1){} return *p + 1;}' fe04
...
test_timeout  'int f(int ms){int *p=0; *p=ms; while(*p>0){} return 0;} int main() {f(0x7fff); return 3;}'
test_value 03 'int main() {char *p=0x1c; *p=2; return *p+1; }'
test_value 04 'int main() {char *p=0x1d; *p=1; return *p+3; }'
test_value 1d 'int f(){return 0;} int main() {char *p=0x1d; *p=3; f(); return p; }'
